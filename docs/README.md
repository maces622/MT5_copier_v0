...
![Cover](6fbaf024-1f0b-4cd4-b4bd-2080d2ab2ccf.png)
## 业务场景
针对金融业务中的跟单业务场景———一些用户希望复制另一些用户的交易，会通过第三方平台将自己的交易账户和他们信任的交易账户进行绑定，带单账户在下单的同时，会将信号发送给跟单账户，并执行相同的交易行为，或按照用户预定的风险控制参数对仓位进行调整后在同一时刻执行相同的交易行为。
...
2. 一个按模块拆分的 Spring Boot 单体应用（便于后续的微服务拆分）
